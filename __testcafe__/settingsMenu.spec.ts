...
{
    await t
        .expect( Selector( `.${CLASSES.SETTINGS_MENU__BUTTON}` ).exists ).ok()
} );
...
test( 'can open settings menu to check options exist', async ( t ) =>
{
    await t
        ...
        .expect( Selector( `.${CLASSES.SETTINGS_MENU__BOOKMARKS}` ).exists ).ok()
        .expect( Selector( `.${CLASSES.SETTINGS_MENU__HISTORY}` ).exists ).ok()
        .expect( Selector( `.${CLASSES.SETTINGS_MENU__TOGGLE}` ).exists ).ok()
} );
...
test( 'hides settings menu after clicking elsewhere', async ( t ) =>
{
    await t
        .click(  `.${CLASSES.SETTINGS_MENU__BUTTON}` )
        .expect( Selector( `.${CLASSES.SETTINGS_MENU}` ).exists ).ok()

    await t.click(  `.${CLASSES.ADDRESS_BAR}` )
        .expect( Selector( `.${CLASSES.SETTINGS_MENU}` ).exists ).notOk()
} );
