...
export class ViewManager {
  ...
  constructor(mainWindow, store) {
    ...
    ipcMain.on(
      'browserview-load',
      (e: Electron.IpcMessageEvent, { url, id }: any) => {
        const view = this.views[id];
    ...
    setInterval(() => {
      for (const key in this.views) {
        ...
        if (title !== view.title) {
          mainWindow.webContents.send(`browserview-data-updated-${key}`, {
            title,
            url
          });
          view.url = url;
  ...
  public create(tabId: number, url: string) {
    const view = new View(tabId, url, this.mainWindow, this);
  ...
  public fixBounds() {
    ...
    view.setBounds({
      x: 0,
      y: this.fullscreen ? 0 : TOOLBAR_HEIGHT + 1 + 50,
      width,
  ...
  sendToAll(name: string, ...args: Array<any>) {
    for (const key in this.views) {
      const view = this.views[key];
