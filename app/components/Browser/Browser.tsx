...
class Browser extends Component<BrowserProps, BrowserState> {
  ...
  componentDidMount() {
    ...
    body.append(div);
    const newTabUrl = 'safe-auth://home';
    addTab({
      url: newTabUrl,
      isActiveTab: true,
  ...
  componentDidUpdate = (prevProps: BrowserProps) => {
    ...
    const prevTabsLen = prevProps.tabs.length;
    const currTabsLen = currentProps.tabs.length;
    if (prevTabsLen !== currTabsLen && currTabsLen - prevTabsLen > 0) {
      const latestTab = currentProps.tabs[currTabsLen - 1];
      ipcRenderer.send('browserview-create', {
        tabId: latestTab.index,
        url: latestTab.url
      });
    }
  };
